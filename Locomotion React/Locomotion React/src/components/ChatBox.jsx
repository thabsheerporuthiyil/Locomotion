...
export default function ChatBox({ rideId, onClose }) {
    ...
    return (
        <div className="fixed inset-0 md:inset-auto md:bottom-4 md:right-4 w-full md:w-96 h-screen md:h-[500px] md:max-h-[80vh] bg-slate-900 md:bg-slate-900/95 md:backdrop-blur-xl rounded-none md:rounded-2xl shadow-2xl shadow-black/50 md:border border-white/10 flex flex-col z-[100] overflow-hidden">
            {/* Header */}
            <div className="bg-slate-950 p-4 text-white flex justify-between items-center shadow-md border-b border-white/5 z-10 relative shrink-0">
                ...
                <div className="flex items-center gap-3 relative z-10">
                    {/* Back arrow — visible on all screen sizes so driver can always exit */}
                    <button 
                        onClick={onClose} 
                        className="flex items-center justify-center p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-full transition-colors"
                        title="Back to rides"
                    >
                ...
                <button onClick={onClose} className="text-slate-400 hover:text-white transition-colors p-1.5 rounded-full hover:bg-white/10 relative z-10" title="Close chat">
                    <X size={20} />
                </button>
            </div>
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 bg-transparent flex flex-col gap-3">
                {loading ? (
                    ...
                    <div className="flex-1 flex flex-col justify-center items-center text-slate-500">
                        ...
                        <p className="text-xs mt-1 opacity-70">Send a message to coordinate pickup!</p>
                    </div>
                ) : (
                    messages.map((msg, idx) => {
                        ...
                        return (
                            <div key={idx} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                                <span className="text-[10px] text-slate-500 font-medium px-2 mb-1">
            ...
            {/* Input Form */}
            <form onSubmit={handleSendMessage} className="p-3 bg-slate-950 border-t border-white/5 flex gap-2 items-center relative z-10 w-full">
                <input
                    type="text"
                    value={newMessage}
                    onChange={(e) => setNewMessage(e.target.value)}
                    placeholder="Type a message..."
                    className="flex-1 bg-slate-800/80 border border-white/10 text-white rounded-full px-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-slate-800 transition-all placeholder-slate-500"
                    disabled={sending}
                />
                <button
                    type="submit"
                    disabled={!newMessage.trim() || sending}
                    className="bg-indigo-600 text-white p-2.5 rounded-full hover:bg-indigo-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0 flex items-center justify-center w-10 h-10 shadow-[0_0_15px_rgba(79,70,229,0.4)] disabled:shadow-none"
                >
